...
void BetaConfig::ReadConfig( std::string configFile )
{
  ...
  else
  {
    ...
    this->SLAC_raw_data = g_key_file_get_boolean( gkf, this->header_group, "SLAC_raw_data", NULL);

    this->enable_SSRL = g_key_file_get_boolean( gkf, this->SSRL_group, this->SSRL_enable_key, NULL);
    this->SSRL_assist_threshold = g_key_file_get_double( gkf, this->SSRL_group, this->SSRL_assist_threshold_key, NULL);
    this->enbale_SSRL_AC_posisiton = g_key_file_get_boolean( gkf, this->SSRL_group, this->SSRL_AC_position_key, NULL);
    this->SSRL_Dynamic_Noise_Enable = g_key_file_get_boolean( gkf, this->SSRL_group, this->SSRL_Dynamic_Noise_Enable_key, NULL);
    this->SSRL_Brute_Forced_Noise_Corr = g_key_file_get_boolean( gkf, this->SSRL_group, this->SSRL_Brute_Forced_Noise_Corr_key, NULL);
